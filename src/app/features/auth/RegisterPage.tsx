import { TextField, Box, Button, Stack, Typography, Snackbar, FormControl, InputLabel, OutlinedInput, InputAdornment, IconButton, FormHelperText } from "@mui/material"
 ...
 */
    ...
    const submitHandler = async (data: registerFormValues) => {
        if (data.confirmPassword !== data.password) {
            setError("confirmPassword", { message: "Confirmation does not match", type: "value" })
            return;
        }
        ...
        try {
            ...
            console.log(error)
            /* setServerIssues(error.data.issues.map((issue: { message: string, field: string }) => issue.message)) */
            error.data.issues.forEach((issue: { message: string, field: "firstName" | "password" | "confirmPassword" | "email" }) => {
    ...
    return (
        <>
            <Box sx={{ height: '100vh', display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "8px" }}>
                ...
                <Typography variant="caption">Already registered? <Link to="/login">LOGIN</Link></Typography>
            </Box>
        </>
    )
}
